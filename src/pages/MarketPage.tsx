...
export default function Market() {
  ...
  const getAllowedIds = (): number[] => {
    ...
    try {
      if (typeof user.discount_cars === "string") {
        return user.discount_cars
          .split(",")
      ...
      if (Array.isArray(user.discount_cars)) {
        return user.discount_cars.map(Number);
      }
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <input
              ...
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search..."
              className="flex-1 md:w-80 px-4 py-2 bg-black/40 border border-white/10 rounded-xl"
            />
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <span
                    className={`font-bold text-lg ${
                      isPromo ? "text-green-400" : "text-white"
                    }`}
                  >
                    ${price.new}
                  </span>
